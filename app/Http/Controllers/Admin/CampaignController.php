...
public function store(Request $request)
{
    ...
    $input = $request->except(['files', 'image']); // Ensure 'image' is excluded
    ...
    if ($image1) {
        ...
        $input['image_one'] = $image1Url; // Assign to input array
    }
    ...
    if ($image2) {
        ...
        $img2->encode('webp', 90);
        $img2->resize(null, 300, function ($constraint) {
            $constraint->aspectRatio();
        });
        $img2->save($image2Url);

        $input['image_two'] = $image2Url; // Assign to input array
    }
    ...
    if ($image3) {
        ...
        $img3->encode('webp', 90);
        $img3->resize(null, 300, function ($constraint) {
            $constraint->aspectRatio();
        });
        $img3->save($image3Url);

        $input['image_three'] = $image3Url; // Assign to input array
    }
